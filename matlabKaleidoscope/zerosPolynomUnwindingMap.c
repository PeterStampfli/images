/*==========================================================
 * zerosPolynomUnwindingMap: Transforms a map using a complex polynom
 * the real and imaginary components of its zeros are given
 * and an amplitude factor
 * An additonal unwinding rotation is done
 *
 * zerosPolynomTransformMap(map, amplitude, realPartZeros, imaginaryPartZeros, unwinding);
 *
 * Input:
 * first the map. 
 ...
 *
 * unwinding, integer number, controls rotation compensating the rotations due to each (z-a_i) factor
 *   get constant phase for large z with unwinding = number of zeros
 * calculates amplitude * (z-a_1) * (z- a_2)* *(z-a_n) * exp(- i * unwinding * phi), z = r * exp(i phi) , matlab indexing, n<=9
 *
        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    float inverted;
    float complex z, w, amplitude, phi, x, y, unwinding;
    ...
    if(nrhs < 5) {
        mexErrMsgIdAndTxt("zerosPolynomTransformMap:nrhs","A map input required, an amplitude and real and imaginary part arrays of zeros, integer unwinding.");
    }
    ...
    unwinding = (float) mxGetScalar(prhs[4]);
