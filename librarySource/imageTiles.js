 ...
 * @param {Vector2} right
 */
imageTiles.addTwoColorParallelogram = function(angle, left, right) {
 ...
 */
imageTiles.addTwoColorPolygon = function(n, firstCorner, secondCorner) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        if (i & 1) {
            imageTiles.polygons.addInvertedImagePolygon(first, second, center);
        } else {
            imageTiles.polygons.addImagePolygon(first, second, center);
        }
...
imageTiles.addSymmetricParallelogram = function(angle, left, right) {
    ...
    imageTiles.polygons.addImagePolygon(left, bottomLeft, centerLeft);
    imageTiles.polygons.addImagePolygon(bottom, bottomRight, centerRight, center);
    imageTiles.polygons.addImagePolygon(right, topRight, centerRight);
    imageTiles.polygons.addImagePolygon(top, topLeft, centerLeft, center);
    imageTiles.polygons.addInvertedImagePolygon(bottomLeft, bottom, center, centerLeft);
    imageTiles.polygons.addInvertedImagePolygon(bottomRight, right, centerRight);
    imageTiles.polygons.addInvertedImagePolygon(topRight, top, center, centerRight);
    imageTiles.polygons.addInvertedImagePolygon(topLeft, left, centerLeft);
};

/**
 * create a "directed" regular polygon, all corners of the same type
 * given number n of sides, a first and a second corner, the polygon lies at left of the line from first to second
...
imageTiles.addSymmetricPolygon = function(n, firstCorner, secondCorner) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        second.set(center).add(centerSecond);
        imageTiles.polygons.addImagePolygon(first, middle, center);
        imageTiles.polygons.addInvertedImagePolygon(middle, second, center);
        centerFirst.rotate(alpha);
