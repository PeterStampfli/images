...
function creation() {
    ...
    let intersectionAngle = Range.create("n");
    intersectionAngle.setRange(0.1, 0.5);
    intersectionAngle.setValue(angle);
    intersectionAngle.onChange = function(x) {
        dihedralOrder.setValue(1 / x);
        Make.updateNewMap();
    };


    intersectionAngle.setStep(0.0001);

    let dihedralOrder = NumberButton.create("order", "minus", "plus");
    dihedralOrder.setFloat(0.01);
    dihedralOrder.setValue(1 / angle);
    dihedralOrder.onChange = function(x) {
        intersectionAngle.setValue(1 / x);
        Make.updateNewMap();
    };
    ...
    Make.initializeMap = function() {
        console.log("init");
        const alpha = Math.PI * intersectionAngle.getValue();
