...
if (DOM.idExists("saveMapStructure")) {
    Make.saveMapStructureButton = new Button('saveMapStructure');
    Make.saveMapStructureButton.onClick = function() {
        const height = Make.map.height;
        const width = Make.map.width;
        const reflectionsArray = Make.map.reflectionsArray;
        const lyapunovArray = Make.map.lyapunovArray;
        let mapData = '%%MatrixMarket matrix array integer general\r\n';
        mapData += '\r\n';
        ...
        mapData += '% an array of the structure of the kaleidoscope mapping\r\n';
        mapData += '% for each pixel an integer: -1 if map has no valid result\r\n';
        mapData += '% 0 for even number of inversions (reflections)\r\n';
        mapData += '% 1 for odd number of inversions (reflections)\r\n';
        mapData += '\r\n';
        ...
        for (var i = 0; i < width; i++) {
            for (var j = 0; j < height; j++) {
                ...
                if (lyapunovArray[index] < 0) {
                    mapData += '-1\r\n';
                } else mapData += (reflectionsArray[index] & 1) + '\r\n';
            }
        }
        const blob = new Blob([mapData], {
            type: 'text/plain'
        });
        const filename = 'mapStructure.txt';
